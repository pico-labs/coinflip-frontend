...
const functions = {
    ...
    setActiveInstanceToLocal: async (_args)=>{
        const Local = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .Mina.LocalBlockchain */ .No.LocalBlockchain();
        ...
        state.isLocal = true;
        state.localAppPrivateKey = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PrivateKey.random */ ._q.random();
    },
    loadContract: async (_args)=>{
        ...
        assertsIsSpecifiedContract(Add, "Add");
        state.Add = Add;
    },
    compileContract: async (_args)=>{
        assertsIsSpecifiedContract(state.Add, "Add");
        await state.Add.compile();
    },
    fetchAccount: async (args)=>{
        if (state.isLocal) {
            console.info("get account with key: ".concat(args.publicKey58));
            const publicKey = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PublicKey.fromBase58 */ .nh.fromBase58(args.publicKey58);
            return snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .Mina.getAccount */ .No.getAccount(publicKey);
        } else {
            const publicKey1 = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PublicKey.fromBase58 */ .nh.fromBase58(args.publicKey58);
    ...
    initLocalZkappInstance: async (args)=>{
        assertsIsSpecifiedContract(state.Add, "Add");
        const userPrivateKey = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PrivateKey.fromBase58 */ ._q.fromBase58(args.userPrivateKey58);
        const appPrivateKey = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PrivateKey.fromBase58 */ ._q.fromBase58(args.appPrivateKey58);
        const instance = new state.Add(snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PublicKey.fromPrivateKey */ .nh.fromPrivateKey(appPrivateKey));
        let tx = await snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .Mina.transaction */ .No.transaction(userPrivateKey, ()=>{
            snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .AccountUpdate.fundNewAccount */ .nx.fundNewAccount(userPrivateKey);
            instance.deploy({
                zkappKey: appPrivateKey
            });
            instance.init();
        });
    ...
    createLocalUpdateTransaction: async (args)=>{
        const feePayerKey = snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .PrivateKey.fromBase58 */ ._q.fromBase58(args.userPrivateKey58);
        const transaction = await snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .Mina.transaction */ .No.transaction({
            feePayerKey,
            fee: 100000000
        }, ()=>{
    ...
    sendLocalTransaction: async (_args)=>{
        const res = await snarkyjs__WEBPACK_IMPORTED_MODULE_0__/* .Mina.sendTransaction */ .No.sendTransaction(state.transaction);
        return res.hash();
    }
};
// ---------------------------------------------------------------------------------------
function assertsIsSpecifiedContract(contract, expectedName) {
    if (typeof contract !== "function") {
        throw "contract is not a function";
    }
    const castContract = contract;
    const namesDiffer = castContract.name !== expectedName;
    if (!castContract.name || namesDiffer) {
        throw "Expected contract to have name '".concat(expectedName, "' but got '").concat(contract.name);
    }
}
if (true) {
...
/******/ 			return "static/chunks/" + (chunkId === 829 ? "cc8f0cfa" : chunkId) + "." + {"189":"1817c0373708f86a","829":"376cd251a17a8950"}[chunkId] + ".js";
/******/ 		};
/******/ 	}();
/******/ 	
